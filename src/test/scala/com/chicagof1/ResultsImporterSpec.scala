...
class ResultsImporterSpec extends FunSpec with Matchers {
  ...
  describe("A Results Importer") {
    it("should read a racer result from csv contents") {
      ...
      result(0) should be(RacerResult(RacerName("Justin"), 1, 16, Period.seconds(33).plusMillis(720).toStandardDuration))
      result(10) should be(RacerResult(RacerName("SuperMario"), 11, 8, Period.seconds(39).plusMillis(690).toStandardDuration))
    }
    ...
    it("should read a racer result with gaps") {
      ...
      result(0) should be(RacerResult(RacerName("Federico Caimi"), 1, 22, Period.seconds(0).toStandardDuration))
      result(1) should be(RacerResult(RacerName("Ken Lim"), 2, 30, Period.seconds(4).plusMillis(893).toStandardDuration))
      result(23) should be(RacerResult(RacerName("James Moeller"), 24, 10, Period.seconds(0).toStandardDuration))
    }
    ...
    it("should read a race from a csv file") {
      ...
      race.results(0) should be(RacerResult(RacerName("Justin"), 1, 16, Period.seconds(33).plusMillis(720).toStandardDuration))
      race.results(10) should be(RacerResult(RacerName("SuperMario"), 11, 8, Period.seconds(39).plusMillis(690).toStandardDuration))
    }
